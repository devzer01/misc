...
return _joiner_js(array('cachedir'=>'/cache',
					    'tag'=>'script',
	...
	if(is_array($script_array)) {
		...
		if($contents) {
			...
			if ($fp = fopen($cachedir . '/' . $cache_file . '.' . $options[ext], 'wb')) {
					...
					//run source thru minifier
					shell_exec("java -jar ".$_SERVER['yuicompressor_path']."\yuicompressor-2.3.4.jar -o $filename --nomunge $filename");
