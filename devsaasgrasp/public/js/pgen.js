...
function update_no_premium(field, i_option, i_country, i_sample) {
	
	var sample 	= parseInt(document.getElementById(field + i_option + "_country_" + i_country + "_" + i_sample).value);
	var total 	= 0;
	
	for (i=1; i<=100; i++) {
		var extra = document.getElementById(field + i_option + "_country_" + i_country + "_" + i_sample + "_" + i);
		if (extra) {
			if(extra.value != '') {
				sample 	= sample - parseInt(extra.value);
	...
	if(sample < 0) {
		sample = 0;
		document.getElementById(field + i_option + "_country_" + i_country + "_" + i_sample).value = total;
		update_no_premium_in_all(field, i_option, i_country, i_sample, total);
	}
	
	var no_premium = document.getElementById(field + i_option + "_country_" + i_country + "_" + i_sample + "_0");
	
	if (no_premium) {
		no_premium.value = sample;
	}
}

function balance_no_premium_completes(i_option, i_country, i_sample) {
	
	var field	= 'completes_option_';
	var total	= 0;
	var remaining_comp	= document.getElementById(field + i_option + "_country_" + i_country + "_" + i_sample + "_0").value;
		
	if(remaining_comp < 0 ) {
		alert('Please enter valid integer, greater than zero.');
		document.getElementById(field + i_option + "_country_" + i_country + "_" + i_sample + "_0").value = 0;
		return false;
	}
	
	total += parseInt(remaining_comp);
...
function update_no_premium_in_all(field, i_option, i_country, i_sample, i_total) {
	
	var total = 0;
	
	if(i_sample == 1 ) {				
		i_plus_sample = document.getElementById(field + i_option + "_country_" + i_country + "_3");
		if(i_plus_sample) {
			total = parseInt(i_total) + parseInt(i_plus_sample.value);	
		}
		
		document.getElementById(field + i_option + "_country_" + i_country + "_0").value = total;
	}else if (i_sample == 3) {		
		i_min_sample = document.getElementById(field + i_option + "_country_" + i_country + "_1" );
		if(i_min_sample) {
			total = parseInt(i_total) + parseInt(i_min_sample.value);	
		}
...
function update_no_premium_completes(i_option, i_country, i_sample) {
	update_no_premium('completes_option_', i_option, i_country, i_sample);
}

function update_no_premium_incidence(i_option, i_country, i_sample) {
	update_no_premium('incidence_option_', i_option, i_country, i_sample);
}

function update_no_premium_length(i_option, i_country, i_sample) {
	update_no_premium('length_option_', i_option, i_country, i_sample);
}

////////////////////////////
function add_quota_row(i_option, i_country, i_sample, proposal_revision_panel_id, row_index)
{
	var country = document.getElementById('country_name_option_' + i_option + '_country_' + i_country);
	var completes = document.getElementById('completes_option_' + i_option + '_country_' + i_country + '_' + i_sample);
	var incidence = document.getElementById('incidence_option_' + i_option + '_country_' + i_country + '_' + i_sample);
	var length = document.getElementById('length_option_' + i_option + '_country_' + i_country + '_' + i_sample);
	
	if ((country.selectedIndex == 0) || (parseInt(completes.value) == 0) || (parseInt(incidence.value) == 0) || (parseInt(length.value) == 0))
	{
		alert('Please enter values: Country Name, Completes, Incidence, Questionnaire Length');
		return;
	}
		
	var labels = new Array('', 'description_option_', 'completes_option_', 'incidence_option_', 'length_option_');
	var values = new Array('', '', completes.value, incidence.value, length.value);
	var table = document.getElementById('panel_details_' + i_option + '_' + i_country);
	
	//this is the number of quota group rows we have manually added			
	var num = parseInt(document.getElementById("description_count_option_" + i_option + "_country_" + i_country + "_" + i_sample).value);
	
	//this is where we do the custom first row	
	if (num == 0) {
	
			var num_quote_rows = document.getElementsByName('row_'+i_option+'_'+i_country+'_'+i_sample).length;
			
			var row_position = row_index + num_quote_rows;
			...
			row.id = 'row_'+i_option+'_'+i_country+'_'+i_sample+'_'+proposal_revision_panel_id+'_'+num;
			row.setAttribute('name', 'row_'+i_option+'_'+i_country+'_'+i_sample+'_'+proposal_revision_panel_id+'_'+num);
			row.className = 'tab';
			...
			for (i=0; i <= 6; i++) {
				...
				//processes like createElement should be moved to a common method, so we just pass in some arguments to a method and get the output object - NH
				switch (i) {
				case 0:					
					var node = document.createElement('input');
					node.type = 'hidden';
					node.value = 0;	
					node.id = 'proposal_revision_panel_id_option_' + i_option + '_country_' +  i_country + '_' + i_sample +'_' + num ;
					node.name = 'proposal_revision_panel_id_option_' + i_option + '_country_' +  i_country + '_' + i_sample +'_' + num ;
					cell.appendChild(node);
					
					var node = document.createElement('input');
					node.type = 'hidden';
					node.value = 0;	
					node.id = 'prime_option_' + i_option + '_country_' +  i_country + '_' + i_sample +'_' + num ;
					node.name = 'prime_option_' + i_option + '_country_' +  i_country + '_' + i_sample +'_' + num ;
					cell.appendChild(node);
				...
				case 1:
					...
					node.id = labels[i] + i_option + '_country_' +  i_country + '_' + i_sample +'_' + num ;
					node.name = labels[i] + i_option + '_country_' +  i_country + '_' + i_sample + '_' + num;
					node.value = "Remaining Completes";
					cell.appendChild(node);
					break;
				...
				case 6:
					html = '<br /><img src="/images/delete.gif" width="8" height="8" title="Delete" align="Delete" style="cursor:pointer" onclick="remove_quota_row(\'row_'+i_option+'_'+i_country+'_'+i_sample+'_'+proposal_revision_panel_id+'_'+num+'\');" />';
					cell.align = 'center';
					cell.innerHTML = html;
					break;
				...
				case 4:
					...
					if (i == 2) {
						if (node.addEventListener){							
							node.addEventListener("keyup", function calling() { balance_no_premium_completes(i_option, i_country, i_sample); }, false);
						} else if (node.attachEvent){
  							node.attachEvent('onkeyup', function calling() { balance_no_premium_completes(i_option, i_country, i_sample); });
						}
					} else {
						node.value = values[i];
					}
	...
	document.getElementById("description_count_option_" + i_option + "_country_" + i_country + "_" + i_sample).value = num + 1;
		
	num_quote_rows = document.getElementsByName('row_'+i_option+'_'+i_country+'_'+i_sample).length;
	
	//looks like microsoft developers dont know how to count and put a 0 based index on a table rows and all that kinda crap. 
	//since there is a building blocking my line of sight towards redmond i am going to just put this hack for now, but one day when 
	//i meet the arms dealer my first purchase of nukes will be GPS guided to 47.777867,-122.099991
	...
	for (i=0; i <= 6; i++) {
			...
			switch (i) {
				case 0:
//					html = country.options[country.selectedIndex].text;
					html = "<input type='hidden' name='proposal_revision_panel_id_option_" + i_option + "_country_" + i_country + "_" + i_sample + "_" + num +"' value='0'>";
					html = html + "<input type='hidden' name='prime_option_" + i_option + "_country_" + i_country + "_" + i_sample + "_" + num + "' value='0'>";
					cell.innerHTML = html;
					break;
				case 1:
					var node = document.createElement('input');
				...
				case 5:
					if (document.getElementById('sample_type_lists_'+i_sample)) {
						html = document.getElementById('sample_type_lists_'+i_sample).innerHTML;
						html = html.replace(/OOOOOO/g, i_option);
						html = html.replace(/CCCCCC/g, i_country);
						html = html.replace(/NNNNNN/g, num);
				...
				case 4:
					...
					if (i == 2) {
						if (node.addEventListener){							
							node.addEventListener("keyup", function calling() { update_no_premium_completes(i_option, i_country, i_sample); }, false);
						} else if (node.attachEvent){
  							node.attachEvent('onkeyup', function calling() { update_no_premium_completes(i_option, i_country, i_sample); });
						}
					} else {
						node.value = values[i];
					}
...
function remove_quota_row(delete_row_id)
{
	$('#'+delete_row_id).remove();
}

////////////////////////////
function copy_country_description(obj, index)
{
	var desc_objects = document.getElementsByName('country_name_' + index);
	
	for (i = 0; i < desc_objects.length; i++) {
		desc_objects[i].innerHTML = "<strong>" + obj.options[obj.selectedIndex].text + "</strong>";
	}
}
...
function copy_textbox_values(o, el, option, country, sample, copyall)
{
	if (option > 1) return true;
	var num_options = document.getElementsByName('number_of_options')[0].value;

	//this should really be its own method especially the part that does a wildcard check on the elementname
	//something like getElementsByNamebyWildCard()
	if (copyall == 1 && sample == 0 && country == 1)
	{
		var objects = document.getElementsByTagName('INPUT');
		var el_length = el.length;
		
		for(i = 0; i < objects.length; i++) {
			if (objects[i].name.substring(0, el_length) == el) {
				objects[i].value = o.value;
			}
		}
	...
	for (i=1; i <= num_options; i++ ) {
		...
		var obj_target = document.getElementsByName(el+'_option_'+i+'_country_'+country+'_'+sample)[0];
		if (obj_target) {
			obj_target.value = o.value;
		}
	}
//	if(sample!=3) calcB2BVals(o, el, option, country, sample);
}

function calcB2BVals(o, el, option, country, sample){
	_all = document.getElementsByName(el+'_option_'+option+'_country_'+country+'_all')[0];
	cons = document.getElementsByName(el+'_option_'+option+'_country_'+country+'_1')[0];
	b2b = document.getElementsByName(el+'_option_'+option+'_country_'+country+'_3')[0];
	result = parseInt(_all.value) - parseInt(cons.value);
	if(!isNaN(result)) b2b.value = result;
   ...
   for (i=1; i <= num_options; i++ ) {
		...
		var obj_target = document.getElementsByName(el+'_option_'+i+'_country_'+country+'_3')[0];
      if (obj_target) {
      	obj_target.value = document.getElementsByName(el+'_option_'+option+'_country_'+country+'_3')[0].value;
   	}
  	}
...
function validate_add_revision(f)
{
   ...
   if (!isServicesSelected()) {
   		submit = 0;
   		msg += 'You must selected at least one service to render \r\n';
   }
   ...
   var level_0_category = document.getElementById('study_category_level_0');
		
	if(level_0_category.value != 0) {
		var level_1_category = document.getElementById('study_category_level_1');
		if(level_1_category.value == 0) {				
			submit = 0;
   		msg += 'Please select a Level 2 Study Category.';				
		}
	}  
...
function isServicesSelected()
{
	var number_of_services = 30;
	
	for (i = 0; i < number_of_services; i++) 
	{
		if (document.getElementById('S_' + i)) {
			if (document.getElementById('S_' + i).checked) {
				return true;
			}
		}	
	}
...
/**
* Set Proposal Cost parameter to zero for multiple countries multiple options
* 
*/
function SetPanelParams(src, option, country)
{		
	var src_value = src.value;
   
	//check whether Sample Source is Third Party Supplier
   if(src_value == 2) {
   	
  		var number_of_countries = document.getElementsByName('number_of_countries')[0];
  		var number_of_options 	= document.getElementsByName('number_of_options')[0];
  		
		//If the Proposal is for Single Country multiple options
   	if(number_of_countries = 1 && number_of_options > 1) {	
   		if (country != 1 || option != 1) return true;	
   				
			for (o=1; o <= number_of_options.value; o++ ) {
		      for (c=1; c <= number_of_countries.value; c++ ) {
		         if (o == 1 && c == 1) continue;		  
		         //set panel cost parameters to zero       		         
		         document.getElementsByName('completes_option_'+o+'_country_'+c)[0].value 					= 0;	
		         document.getElementsByName('incidence_rate_option_'+o+'_country_'+c)[0].value 			= 0;	         
		         document.getElementsByName('questions_programmed_option_'+o+'_country_'+c)[0].value 	= 0;	
		         document.getElementsByName('questions_per_screener_option_'+o+'_country_'+c)[0].value 	= 0;	
		         document.getElementsByName('questions_per_interview_option_'+o+'_country_'+c)[0].value = 0;	         
		      }
       	}       	
       	return true;		  
			...
			//set panel cost parameters to zero   		
			document.getElementsByName('completes_option_'+option+'_country_'+country)[0].value 					= 0;
			document.getElementsByName('incidence_rate_option_'+option+'_country_'+country)[0].value 				= 0;
			document.getElementsByName('questions_programmed_option_'+option+'_country_'+country)[0].value 		= 0;
			document.getElementsByName('questions_per_screener_option_'+option+'_country_'+country)[0].value 	= 0;
			document.getElementsByName('questions_per_interview_option_'+option+'_country_'+country)[0].value 	= 0;
...
function AjaxSaveComment(proposal_id)
{
	...
	var proposal_comment_text = escape(document.getElementById('proposal_comment_text').value);
...
//			alert(src.options[i].text);
	...
	/**
	 * This function checks to see if a checkbox that triggered this function is part of a peticular option, the option is represted by the option parameter
	 *
	 * @param Object obj the check box object
	 * @param int option The number of the option
	 */
	function CheckOptionSelected(obj, option)
	{
		if (!obj.checked) return true;
	
		var options = document.getElementsByName('selected_option');
		
		for (i=0; i < options.length; i++) {
			if (options[i].checked && options[i].value == option) {
				return true;
			}
		}
		
		obj.checked = false;
		alert('You can not selected a country from an option which you have not selected \n Please select the Option Number ' + option + ' Before selecting this country ');
		return false;
	}
	
	/**
	 * when marking a proposal as won, this function clears out any selected text boxes, when the won option has been changed.
	 * used in vw_proposal_won.tpl and vw_custom_won.tpl reffer to HB-591 regarding some issues that happend because of this validation was missing
	 */
	function ClearSelectedCountries()
	{
		//clear selected countries
		var countries = document.getElementsByName('selected_country[]');
	...
	/**
	 * Select the list boxes depending on the value selected in sample types multiple list box
	 * 
	 */
	function DisplayListBoxes(selObj)
	{
		var map = new Array();
		map[1]="Consumer";
		map[4]="Consumer_Medical";
		map[5]="Consumer";
		map[2]="Medical";
		map[3]="IT";
		map[6]="B2B";
		...
		var selectedArray = new Array();
  		...
  		for (i=0; i<selObj.options.length; i++) {
    		if (selObj.options[i].selected) {
    			count++;
    			currVal = selObj.options[i].value;
      			div_name = 'div_'+map[currVal];
      			$('#'+div_name).show();
    		}
  		} 
  		/*
  		if(count>0){
  			document.getElementById('categories_info').style.visibility = 'hidden';
  		} else {
  			document.getElementById('categories_info').style.visibility = 'visible';
  		}*/
