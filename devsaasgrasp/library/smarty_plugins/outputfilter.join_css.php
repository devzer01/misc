...
return _joiner(array('cachedir'=>'/cache',
					  'tag'=>'link',
					  'type'=>'text/css',
					  'ext'=>'css',
					  'src'=>'href',
...
function _joiner($options,$source) {
	...
	if(is_array($script_array)) {
		...
		if($contents) {
			$filename = $cachedir . '/' . $cache_file . '.' . $options[ext];
			if ($fp = fopen($cachedir . '/' . $cache_file . '.' . $options[ext], 'wb')) {
					fwrite($fp, $contents);
					fclose($fp);

					//shell_exec("java -jar e:\websites\yuicompressor\build\yuicompressor-2.3.4.jar -o $filename $filename");
					//die("|--->".getenv("yuicompressor_path"));
					//die("-->".$_ENV['yuicompressor_path']);
					shell_exec("java -jar ".$_SERVER['yuicompressor_path']."\yuicompressor-2.3.4.jar -o $filename $filename");
