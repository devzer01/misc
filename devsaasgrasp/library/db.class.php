...
class db {
	...
	*/
	function __construct($params = array())
	{
	   ...
	   $this->__default_params = array('host' => $this->config->host, 'username' => $this->config->user, 'password' => $this->config->pass, 'dbname' => $this->config->name);
			   
	   //$params = array_merge($params, $this->__default_params);		
	   $params = $this->__default_params;		
		
		if ($params['host'] == 'localhost' && is_php52()) {
		...
		if (!isset($config->database->version)) {
			$this->version = "5.0";
		}
