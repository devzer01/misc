...
<form name='commercial_agreement' 
      id='commercial_agreement' 
      action='?e={"action=save_commercial_agreement&account_id=$account_id"|url_encrypt}'
      method="POST"
      enctype="multipart/form-data">
<a name="commercial_agreement"></a>
{include file='common/div_header.tpl' 
			div_title='Commercial Agreement' 
			div_name='commercial_agreement' 
			div_width='80%' 
			div_align='center'}
	<table width="100%">
   <tr class="header1">
    ...
    {section name=id loop=$comments.comterms_agreement}
   <tr class="{cycle values='tab,tab1'}">
      <td>{$comments.comterms_agreement[id].created_by_name}</td>
      <td>{$comments.comterms_agreement[id].created_date|date_format:"%Y-%m-%d"}</td>
      <td>{$comments.comterms_agreement[id].comment_text|nl2br}</td>
   </tr>
{sectionelse}
	<tr class="tab1">
		<td colspan="9">
 ...
 <tr class="header1">
   ...
   <tr class="tab1">
      <td colspan="6" align="center">
         ...
         &nbsp;&nbsp;
         <input type="button" 
                onclick="window.location.href ='?e={"action=display_account_detail&account_id=$account_id"|url_encrypt}#commercial_agreement';"
                value="Cancel">
      </td>
   </tr> 
   </TABLE>  
