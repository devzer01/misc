...
<form method='post' action='/quote/savedetail/id/{$quote.proposal_id}'>
<div style='border: 1px solid black; width: 800px;'>	
<div style='clear:both; float: left;'><strong>Quote</strong>: {$quote.proposal_name}</div>
<div style='float: right;'><strong>PO Number</strong>: <input type='text' size='20' name='ponumber' id='ponumber' value='{$quote.ponumber}'/> </div>
<div style='clear:both; float: left;'><strong>Account</strong>: {$quote.account_name}</div>
<div style='float: right;'>
	<strong>Date</strong>: 
	<input type='text' size='20' name='quotedate' id='quotedate' value='{$quote.quote_date}' /> <button type="button" id="show" title="Show Calendar"><img src='/images/calbtn.gif' /></button>
	<div id="cal1Container"></div>
</div>
<div style='clear:both; float: left;'><strong>Contact</strong>: {$contact.contact_name}</div>
<div style='float: right;'><strong>Discount</strong>:<input style='text-align: right;' type='text' size='5' name='discount' id='discount' value='{$quote.discount}'/></div>
</div>
...
<div style='clear: both; width: 100px;'>
	<div id="divsku" class="yui3-skin-sam">
...
<div style='float: left;'><textarea name='terms' style='width: 300px;'>{$terms.comment_text}</textarea></div>
</div>
<div style='width: 45%; float: left; border: 1px solid black;'>
<div style='width: 100%; float: left;'>Message</div>
<div style='float: left; width: 100%; height: 60px;'><textarea name='notes' style='width: 90%; height: 90%;'>{$notes.comment_text}</textarea></div>
</div></div>
<div style='clear:both;'>
	<input type='button' value='Save Quote' onclick='this.form.submit();' />
...
{literal}
	...
	YUI().use('autocomplete', 'autocomplete-highlighters', function (Y) {
		  Y.one('#sku_1').plug(Y.Plugin.AutoComplete, {
		    resultHighlighter: 'phraseMatch',
		    resultListLocator: 'skus',
		    resultTextLocator: 'sku_name',
		    queryDelay: 300,
		    source: '/quote/skulookup/name/{query}/callback/{callback}'
		  });
	});
